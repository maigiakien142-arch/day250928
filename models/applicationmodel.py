...
class Application(BasicModel, table=True):
    ...
    gpu_server: GPUServer = Field(default=UrgencyLevel.low, description="gpu服务器是否可用，0可用，1代表不可用",index=True)
    start_number: int = Field(description="起始编号")
    description: str = Field(description="描述", max_length=255)
    contribution_score: str = Field(description="贡献得分", max_length=50)
    research_score: str = Field(description="科研得分", max_length=50)
    efficiency_score: str = Field(description="效率得分", max_length=50)
    comprehensive_score: str = Field(description="综合得分", max_length=50)
